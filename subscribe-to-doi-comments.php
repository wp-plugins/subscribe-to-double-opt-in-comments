...
<?php /* ------------------------------------------------------------------- */ ?>
	...
	<label for="subscribe"><?php __($sg_subscribe->not_subscribed_text); ?></label>
	</p>
...
<?php /* ------------------------------------------------------------- */ ?>
	...
	<p <?php if ($sg_subscribe->clear_both) echo 'style="clear: both;" '; ?>class="subscribe-to-doi-comments">
	<?php __(str_replace('[manager_link]', $sg_subscribe->manage_link($email, true, false), $sg_subscribe->author_text)); ?>
	</p>
...
<?php /* --------------------------------------------------------------- */ ?>
	...
	<p <?php if ($sg_subscribe->clear_both) echo 'style="clear: both;" '; ?>class="subscribe-to-doi-comments">
	<?php __(str_replace('[manager_link]', $sg_subscribe->manage_link($email, true, false), $sg_subscribe->subscribed_text)); ?>
	</p>
...
class sg_subscribe {
	...
	function solo_subscribe ($email, $postid) {
		...
		if ( !is_email($email) ) {
			...
			if ( is_email($user_email) )
				...
				$this->add_error(__('Please provide a valid e-mail address.', 'subscribe-to-doi-comments'),'solo_subscribe');
		}

		if ( ( $email == $this->site_email && is_email($this->site_email) ) || ( $email == get_settings('admin_email') && is_email(get_settings('admin_email')) ) )
			$this->add_error(__('This e-mail address may not be subscribed', 'subscribe-to-doi-comments'),'solo_subscribe');
		...
		if ( !$post )
			$this->add_error(__('Comments are not allowed on this entry.', 'subscribe-to-doi-comments'),'solo_subscribe');
		...
		if ( !is_array($this->errors['solo_subscribe']) ) {
			add_post_meta($postid, '_sg_subscribe-to-doi-comments', $email);
			setcookie('comment_author_email_' . COOKIEHASH, $email, time() + 30000000, COOKIEPATH);
	...
	function change_email() {
		...
		if ( $wpdb->query("UPDATE $wpdb->comments SET comment_author_email = '$new_email' WHERE comment_author_email = '$email'") )
			$return = true;
		if ( $wpdb->query("UPDATE $wpdb->postmeta SET meta_value = '$new_email' WHERE meta_value = '$email' AND meta_key = '_sg_subscribe-to-doi-comments'") )
			$return = true;
		return $return;
	}
...
function sg_subscribe_start() {
	...
	if ( !$sg_subscribe ) {
		load_plugin_textdomain('subscribe-to-doi-comments');
...
function sg_subscribe_admin($standalone = false) {
	...
	if ( !$sg_subscribe->validate_key() && strlen($_GET['verify']) != 15 )
		die ( __('You may not access this page without a valid key.', 'subscribe-to-doi-comments') );
	...
	switch ($sg_subscribe->action) :
		...
		case "change_email" :
			if ( $sg_subscribe->change_email() ) {
				$sg_subscribe->add_message(sprintf(__('All notifications that were formerly sent to <strong>%1$s</strong> will now be sent to <strong>%2$s</strong>!', 'subscribe-to-doi-comments'), $sg_subscribe->email, $sg_subscribe->new_email));
		...
		case "remove_block" :
			if ( $sg_subscribe->remove_block($sg_subscribe->email) )
				$sg_subscribe->add_message(sprintf(__('The block on <strong>%s</strong> has been successfully removed.', 'subscribe-to-doi-comments'), $sg_subscribe->email));
			else
				$sg_subscribe->add_error(sprintf(__('<strong>%s</strong> isn\'t blocked!', 'subscribe-to-doi-comments'), $sg_subscribe->email), 'manager');
			break;

		case "email_change_request" :
			if ( $sg_subscribe->is_blocked($sg_subscribe->email) )
				$sg_subscribe->add_error(sprintf(__('<strong>%s</strong> has been blocked from receiving notifications.  You will have to have the administrator remove the block before you will be able to change your notification address.', 'subscribe-to-doi-comments'), $sg_subscribe->email));
			else
				if ($sg_subscribe->change_email_request($sg_subscribe->email, $sg_subscribe->new_email))
					$sg_subscribe->add_message(sprintf(__('Your change of e-mail request was successfully received.  Please check your old account (<strong>%s</strong>) in order to confirm the change.', 'subscribe-to-doi-comments'), $sg_subscribe->email));
			break;

		case "block_request" :
			if ($sg_subscribe->block_email_request($sg_subscribe->email ))
				$sg_subscribe->add_message(sprintf(__('Your request to block <strong>%s</strong> from receiving any further notifications has been received.  In order for you to complete the block, please check your e-mail and click on the link in the message that has been sent to you.', 'subscribe-to-doi-comments'), $sg_subscribe->email));
			break;

		case "solo_subscribe" :
			$sg_subscribe->add_message(sprintf(__('<strong>%1$s</strong> has been successfully subscribed to %2$s', 'subscribe-to-doi-comments'), $sg_subscribe->email, $sg_subscribe->entry_link($_GET['subscribeid'])));
			break;
	...
	<html>
	<head>
	<title><?php printf(__('%s Comment Subscription Manager', 'subscribe-to-doi-comments'), bloginfo('name')); ?></title>
	...
	<div class="wrap">
	<h2><?php printf(__('%s Comment Subscription Manager', 'subscribe-to-doi-comments'), bloginfo('name')); ?></h2>

	<?php if (!empty($sg_subscribe->ref)) : ?>
	<?php $sg_subscribe->add_message(sprintf(__('Return to the page you were viewing: %s', 'subscribe-to-doi-comments'), $sg_subscribe->entry_link(url_to_postid($sg_subscribe->ref), $sg_subscribe->ref))); ?>
	<?php $sg_subscribe->show_messages(); ?>
	<?php endif; ?>
	...
	<?php if ( $sg_subscribe->is_blocked() ) { ?>
		...
		<fieldset class="options">
			<legend><?php _e('Remove Block', 'subscribe-to-doi-comments'); ?></legend>

			<p>
			<?php printf(__('Click the button below to remove the block on <strong>%s</strong>.  This should only be done if the user has specifically requested it.', 'subscribe-to-doi-comments'), $sg_subscribe->email); ?>
			</p>
	...
	<?php $sg_subscribe->hidden_form_fields(); ?>
			...
			<p class="submit">
			<input type="submit" name="submit" value="<?php _e('Remove Block &raquo;', 'subscribe-to-doi-comments'); ?>" />
			</p>
			</form>
		</fieldset>
	...
	<?php else : ?>
		...
		<fieldset class="options">
			<legend><?php _e('Blocked', 'subscribe-to-doi-comments'); ?></legend>

			<p>
			<?php printf(__('You have indicated that you do not wish to receive any notifications at <strong>%1$s</strong> from this site. If this is incorrect, or if you wish to have the block removed, please contact the <a href="mailto:%2$s">site administrator</a>.', 'subscribe-to-doi-comments'), $sg_subscribe->email, $sg_subscribe->site_email); ?>
			</p>
		</fieldset>
...
<?php
		if ( isset($sg_subscribe->email) && !is_array($postlist) && $sg_subscribe->email != $sg_subscribe->site_email && $sg_subscribe->email != get_bloginfo('admin_email') ) {
			if ( is_email($sg_subscribe->email) )
				$sg_subscribe->add_error(sprintf(__('<strong>%s</strong> is not subscribed to any posts on this site.', 'subscribe-to-doi-comments'), $sg_subscribe->email));
			else
				$sg_subscribe->add_error(sprintf(__('<strong>%s</strong> is not a valid e-mail address.', 'subscribe-to-doi-comments'), $sg_subscribe->email));
		}
?>
	...
	<?php if ( current_user_can('manage_options') ) { ?>
		...
		<fieldset class="options">
			...
			<legend><?php _e('Find Subscriptions', 'subscribe-to-doi-comments'); ?></legend>

			<p>
			<?php _e('Enter an e-mail address to view its subscriptions or undo a block.', 'subscribe-to-doi-comments'); ?>
			</p>
			...
			<input type="submit" value="<?php _e('Search &raquo;', 'subscribe-to-doi-comments'); ?>" />
			</p>
			</form>
		</fieldset>
...
<?php if ( !$_REQUEST['email'] ) : ?>
		<fieldset class="options">
			<?php if ( !$_REQUEST['showallsubscribers'] ) : ?>
				<legend><?php _e('Top Subscriber List', 'subscribe-to-doi-comments'); ?></legend>
			<?php else : ?>
				<legend><?php _e('Subscriber List', 'subscribe-to-doi-comments'); ?></legend>
			<?php endif; ?>
...
//-->
		...
		<fieldset class="options">
			<legend><?php _e('Subscriptions', 'subscribe-to-doi-comments'); ?></legend>

				<p>
				<?php printf(__('<strong>%s</strong> is subscribed to the posts listed below. To unsubscribe to one or more posts, click the checkbox next to the title, then click "Remove Selected Subscription(s)" at the bottom of the list.', 'subscribe-to-doi-comments'), $sg_subscribe->email); ?>
				</p>
	...
	<?php $sg_subscribe->hidden_form_fields(); ?>
				...
				<p>
				<a href="javascript:;" onclick="checkAll(document.getElementById('removeSubscription')); return false; "><?php _e('Invert Checkbox Selection', 'subscribe-to-doi-comments'); ?></a>
				</p>

				<p class="submit">
				<input type="submit" name="submit" value="<?php _e('Remove Selected Subscription(s) &raquo;', 'subscribe-to-doi-comments'); ?>" />
				</p>
				</form>
		</fieldset>
	</div>
	...
	<div class="wrap">
	<h2><?php _e('Advanced Options', 'subscribe-to-doi-comments'); ?></h2>

		<fieldset class="options">
			<legend><?php _e('Block All Notifications', 'subscribe-to-doi-comments'); ?></legend>
	...
	<?php $sg_subscribe->hidden_form_fields(); ?>
				...
				<p>
				<?php printf(__('If you would like <strong>%s</strong> to be blocked from receiving any notifications from this site, click the button below.  This should be reserved for cases where someone is signing you up for notifications without your consent.', 'subscribe-to-doi-comments'), $sg_subscribe->email); ?>
				</p>

				<p class="submit">
				<input type="submit" name="submit" value="<?php _e('Block Notifications &raquo;', 'subscribe-to-doi-comments'); ?>" />
				</p>
				</form>
		</fieldset>

		<fieldset class="options">
			<legend><?php _e('Change E-mail Address', 'subscribe-to-doi-comments'); ?></legend>
	...
	<?php $sg_subscribe->hidden_form_fields(); ?>
				...
				<p>
				<?php printf(__('If you would like to change the e-mail address for your subscriptions, enter the new address below.  You will be required to verify this request by clicking a special link sent to your current address (<strong>%s</strong>).', 'subscribe-to-doi-comments'), $sg_subscribe->email); ?>
				</p>

				<p>
				<?php _e('New E-mail Address:', 'subscribe-to-doi-comments'); ?> 
				<input name="new_email" type="text" id="new_email" size="40" />
				</p>

				<p class="submit">
				<input type="submit" name="submit" value="<?php _e('Change E-mail Address &raquo;', 'subscribe-to-doi-comments'); ?>" />
				</p>
				</form>
		</fieldset>
