...
?>
    ...
    <?php
	  ...
	  // todo remove: add_action('edit_comment', array('sg_subscribe', 'on_edit'));
	  add_action('edit_comment', create_function('$a', 'global $sg_subscribe; sg_subscribe_start(); return $sg_subscribe->on_edit($a);'));
  ...
  $sg_subscribe->solo_subscribe(stripslashes($_POST[\'email\']), (int) $_POST[\'postid\']);
      ...
      function sg_subscribe_admin($standalone = false) {
          ...
          case "email_change_request":
              ...
              elseif ($sg_subscribe->change_email_request($sg_subscribe->email, $sg_subscribe->new_email))
...
?>
    ...
    <fieldset class="options">
      <legend><?php
                      _e('Remove Block', 'subscribe-to-doi-comments');
?></legend>

      <p>
      <?php
                  printf(__('Click the button below to remove the block on <strong>%s</strong>.  This should only be done if the user has specifically requested it.', 'subscribe-to-doi-comments'), $sg_subscribe->email);
?>
      </p>

      <form name="removeBlock" method="post" action="<?php
                  echo $sg_subscribe->form_action;
?>">
      <input type="hidden" name="removeBlock" value="removeBlock /">
  <?php
                  $sg_subscribe->hidden_form_fields();
?>

      <p class="submit">
      <input type="submit" name="submit" value="<?php
                  _e('Remove Block &raquo;', 'subscribe-to-doi-comments');
?>" />
      </p>
      </form>
    </fieldset>
...
?>
    ...
    <fieldset class="options">
      <legend><?php
                      _e('Blocked', 'subscribe-to-doi-comments');
?></legend>
...
?>
  ...
  <?php
                      $postlist = $sg_subscribe->subscriptions_from_email();
?>
...
?>
      ...
      <legend><?php
                          _e('Find Subscriptions', 'subscribe-to-doi-comments');
?></legend>

      <p>
      <?php
                          _e('Enter an e-mail address to view its subscriptions or undo a block.', 'subscribe-to-doi-comments');
?>
      </p>

      <form name="getemail" method="post" action="<?php
                          echo $sg_subscribe->form_action;
?>">
      <input type="hidden" name="ref" value="<?php
                          echo $sg_subscribe->ref;
?>" />

      <p>
      <input name="email" type="text" id="email" size="40" />
      <input type="submit" value="<?php
                          _e('Search &raquo;', 'subscribe-to-doi-comments');
?>" />
      </p>
      </form>
    </fieldset>
...
?>
        <legend><?php
                                  _e('Top Subscriber List', 'subscribe-to-doi-comments');
?></legend>
      <?php
                          else
                              :
?>
        <legend><?php
                              _e('Subscriber List', 'subscribe-to-doi-comments');
?></legend>
      <?php
                          endif;
?>
...
?></a></p>
  <p><textarea cols="60" rows="10"><?php
                          echo implode(', ', array_keys($all_subscriptions));
?></textarea></p>
<?php
                          endif;
...
?>
        <legend><?php
                              _e('Top Subscribed Posts', 'subscribe-to-doi-comments');
?></legend>
        <?php
...
?>
  ...
  <?php
                      if (count($postlist) > 0 && is_array($postlist)) {
?>
...
//-->
    ...
    <fieldset class="options">
      <legend><?php
                          _e('Subscriptions', 'subscribe-to-doi-comments');
?></legend>

        <p>
        <?php
                          printf(__('<strong>%s</strong> is subscribed to the posts listed below. To unsubscribe to one or more posts, click the checkbox next to the title, then click "Remove Selected Subscription(s)" at the bottom of the list.', 'subscribe-to-doi-comments'), $sg_subscribe->email);
?>
        </p>

        <form name="removeSubscription" id="removeSubscription" method="post" action="<?php
                          echo $sg_subscribe->form_action;
?>">
...
?>
          <li><label for="subscrip-<?php
                              echo $i;
?>"><input id="subscrip-<?php
                              echo $i;
?>" type="checkbox" name="subscrips[]" value="<?php
                              echo $postlist[$i];
?>" /> <?php
                              echo $sg_subscribe->entry_link($postlist[$i]);
?></label></li>
        <?php
                          }
?>
...
?></a>
        ...
        <p class="submit">
        <input type="submit" name="submit" value="<?php
                          _e('Remove Selected Subscription(s) &raquo;', 'subscribe-to-doi-comments');
?>" />
        </p>
        </form>
    </fieldset>
  ...
  <div class="wrap">
  <h2><?php
                          _e('Advanced Options', 'subscribe-to-doi-comments');
?></h2>

    <fieldset class="options">
      <legend><?php
                          _e('Block All Notifications', 'subscribe-to-doi-comments');
?></legend>

        <form name="blockemail" method="post" action="<?php
                          echo $sg_subscribe->form_action;
?>">
...
?>
        ...
        <p>
        <?php
                          printf(__('If you would like <strong>%s</strong> to be blocked from receiving any notifications from this site, click the button below.  This should be reserved for cases where someone is signing you up for notifications without your consent.', 'subscribe-to-doi-comments'), $sg_subscribe->email);
?>
        </p>

        <p class="submit">
        <input type="submit" name="submit" value="<?php
                          _e('Block Notifications &raquo;', 'subscribe-to-doi-comments');
?>" />
        </p>
        </form>
    </fieldset>
    ...
    <fieldset class="options">
      <legend><?php
                          _e('Change E-mail Address', 'subscribe-to-doi-comments');
?></legend>

        <form name="changeemailrequest" method="post" action="<?php
                          echo $sg_subscribe->form_action;
?>">
...
?>
        ...
        <p>
        <?php
                          printf(__('If you would like to change the e-mail address for your subscriptions, enter the new address below.  You will be required to verify this request by clicking a special link sent to your current address (<strong>%s</strong>).', 'subscribe-to-doi-comments'), $sg_subscribe->email);
?>
        </p>

        <p>
        <?php
                          _e('New E-mail Address:', 'subscribe-to-doi-comments');
?> 
        ...
        <p class="submit">
        <input type="submit" name="submit" value="<?php
                          _e('Change E-mail Address &raquo;', 'subscribe-to-doi-comments');
?>" />
        </p>
        </form>
    </fieldset>
